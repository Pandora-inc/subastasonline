...
foreach ($subasta->getLotes() as $lote) {
    ...
    $html .= "<div class='objetfit'><img src='https://martinsarachaga.com/imagenes_lotes/" . $lote->getImagenCatalogo() . "' border='0' title'" . $lote->getTitulo() . "' id='imgsubastas'></div>";
    ...
    $html .= "</a>";
    if (!$subasta->isCerrada() && $_SESSION['token'] != null) {
        
        $html .= "<div class='inputOfertar'>";
        $html .= "<input type='number' class='input-text' name='ofert_" . $lote->getId() . "' id='ofert_" . $lote->getId() . "' placeholder='' value='". $lote->getNuevaOferta() . "' required='required'>";
        $html .= "</div>";
        $html .= "<div class='inputOfertar'>";
        $html .= "<button class='btnofertar' onclick='ofertar(" . $lote->getId() . ");'>OFERTAR</button>";
        $html .= "</div>";
    }
    $html .= "</div>";
    $html .= "<!-- FINAL LOTE -->";
